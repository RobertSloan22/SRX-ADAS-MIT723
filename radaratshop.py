...
def draw_radar(frame, cap):
    # Set up radar parameters
    circle_spacing = 50
    circle_radius = 10
    num_circles = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT) / circle_spacing)
    line_length = 150
    line_x = 0
    line_speed = 10

    # Create a blank radar image
    radar = np.zeros_like(frame)

    # Draw radar circles
    for i in range(num_circles):
        radius = circle_radius + i * circle_spacing
        cv2.circle(radar, (int(cap.get(cv2.CAP_PROP_FRAME_WIDTH) / 2), int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))), radius, (255, 255, 255), thickness=2)

    # Update and draw lines
    line_x += line_speed
    if line_x > cap.get(cv2.CAP_PROP_FRAME_WIDTH):
        line_x = 0
    cv2.line(radar, (int(cap.get(cv2.CAP_PROP_FRAME_WIDTH) / 2), int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))), (line_x, int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT)) - line_length), (0, 255, 0), thickness=2)

    # Update and draw circles
    circles = []
    for i in range(num_circles):
        circle = {"radius": circle_radius + i * circle_spacing, "center": (int(cap.get(cv2.CAP_PROP_FRAME_WIDTH) / 2), int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT)))}
        circles.append(circle)
        for circle in circles:
            if isinstance(circle, tuple) and len(circle) == 2:
                center, radius = circle
                cv2.circle(frame, center, radius, (255, 0, 0), thickness=2)


    # Remove old circles
    circles = [circle for circle in circles if circle["radius"] < cap.get(cv2.CAP_PROP_FRAME_WIDTH) / 2]

    # Draw radar image on top of frame
    frame = cv2.addWeighted(frame, 0.8, radar, 0.2, 0)
...
while True:
    ...
    frame, circles = draw_radar(frame, cap)
    for circle in circles:
        cv2.circle(frame, circle[0], circle[1], (255, 0, 0), thickness=2)
